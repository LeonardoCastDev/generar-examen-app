...
class ExamGenerator {
     ...
     */
    public function agregarPregunta($id_materia, $enunciado, $tipo_pregunta, $nivel_dificultad, $puntuacion, $opciones = null, $respuestas = null) {
        try {
            $this->pdo->beginTransaction();
            
            // Validar datos de entrada
            if (empty($id_materia) || empty($enunciado) || empty($tipo_pregunta) || empty($nivel_dificultad)) {
                throw new Exception("Faltan datos obligatorios para agregar la pregunta");
            }
            ...
            $tipos_validos = ['multiple', 'verdadero_falso', 'abierta'];
            if (!in_array($tipo_pregunta, $tipos_validos)) {
                throw new Exception("Tipo de pregunta no válido");
            }
            
            // Validar nivel de dificultad
            $niveles_validos = ['facil', 'medio', 'dificil'];
            if (!in_array($nivel_dificultad, $niveles_validos)) {
                throw new Exception("Nivel de dificultad no válido");
            }
            
            // Validar puntuación
            if ($puntuacion <= 0 || $puntuacion > 10) {
                throw new Exception("La puntuación debe estar entre 0.1 y 10");
            }
            
            // Insertar la pregunta en el banco
            $stmt = $this->pdo->prepare("
                INSERT INTO banco_preguntas (id_materia, enunciado, tipo_pregunta, nivel_dificultad, puntuacion, fecha_creacion, activa)
                VALUES (?, ?, ?, ?, ?, NOW(), 1)
            ");
            ...
            $id_pregunta_banco = $this->pdo->lastInsertId();
            ...
            if ($tipo_pregunta === 'multiple' && !empty($opciones)) {
                if (count($opciones) < 2) {
                    throw new Exception("Las preguntas múltiples deben tener al menos 2 opciones");
                }
                
                $tiene_correcta = false;
                
                foreach ($opciones as $opcion) {
                    if (!isset($opcion['letra']) || !isset($opcion['texto']) || !isset($opcion['correcta'])) {
                        throw new Exception("Formato de opción inválido");
                    }
                    ...
                    $stmt = $this->pdo->prepare("
                        INSERT INTO opciones_banco (id_pregunta_banco, letra_opcion, texto_opcion, es_correcta)
                        VALUES (?, ?, ?, ?)
                    ");
                ...
                if (!$tiene_correcta) {
                    throw new Exception("Debe haber al menos una opción correcta");
                }
            }
            
            // Procesar respuestas para preguntas abiertas y verdadero/falso
            if (($tipo_pregunta === 'abierta' || $tipo_pregunta === 'verdadero_falso') && !empty($respuestas)) {
                foreach ($respuestas as $respuesta) {
                    if (!isset($respuesta['texto']) || !isset($respuesta['exacta'])) {
                        throw new Exception("Formato de respuesta inválido");
                    }
                    
                    $stmt = $this->pdo->prepare("
                        INSERT INTO respuestas_banco (id_pregunta_banco, respuesta_correcta, es_exacta)
                        VALUES (?, ?, ?)
                    ");
            ...
            return [
                'success' => true,
                'message' => 'Pregunta agregada correctamente al banco',
                'id_pregunta_banco' => $id_pregunta_banco
            ];
            
        } catch (Exception $e) {
            $this->pdo->rollBack();
            error_log("Error agregando pregunta: " . $e->getMessage());
            return [
                'success' => false,
                'message' => 'Error al agregar la pregunta: ' . $e->getMessage()
            ];
        }
    }
